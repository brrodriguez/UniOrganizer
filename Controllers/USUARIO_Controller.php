    ...
    case $strings['Insertar']:
		...
		if ($_REQUEST['user'] == "admin") {
            if (!isset($_REQUEST['nombre'])) { //Si no se ha introducido ningun valor, mostramos la vista con el formulario
                ...
                $usuario = get_data_form(); //Recogemos los datos del formulario

                $respuesta = $usuario->Insertar();
                new Mensaje($respuesta, 'USUARIO_Controller.php');
            }
        }else{
			if ($_REQUEST['user'] == "estudiante") {
				if (!isset($_REQUEST['nombre'])) { //Si no se ha introducido ningun valor, mostramos la vista con el formulario
					new USUARIO_Insertar('USUARIO_Controller.php?user=estudiante', 2);
				} else {
					$estudiante = get_data_form(); //Recogemos los datos del formulario

					$respuesta = $estudiante->Insertar();
					new Mensaje($respuesta, 'USUARIO_Controller.php');
				}
			}	
		}
    ...
    case $strings['Borrar']:
        ...
        if (ConsultarTipoUsuario($_REQUEST['username']) == 1) {
            ...
            if (!isset($_REQUEST['nombre'])) {
                ...
                $usuario = new USUARIO_Modelo($_REQUEST['username'], '', ConsultarTipoUsuario($_REQUEST['username']), '', '', '', '', '', '', '', '');
                $valores = $usuario->RellenaDatos();
                if (!tienePermisos('ESTUDIANTE_Borrar')) {
                    new Mensaje('No tienes los permisos necesarios', 'USUARIO_Controller.php');
                } else {
                    //muestra el formulario de borrado
                    new ESTUDIANTE_Borrar($valores, 'USUARIO_Controller.php');
                }
            } else {
                $_REQUEST['password'] = '';
                $estudiante = get_data_form_Entrenador();
                $respuesta = $estudiante->Borrar();
                new Mensaje($respuesta, 'USUARIO_Controller.php');
            }
        }
        break;
    ...
    case $strings['Ver']:
        ...
        if (ConsultarTipoUsuario($_REQUEST['username']) == 1) {
            ...
            if (!isset($_REQUEST['nombre'])) {
                ...
                $valores = $usuario->RellenaDatos();
                $datos = '';
                if (!tienePermisos('ESTUDIANTE_SELECT_SHOW')) {
                    new Mensaje('No tienes los permisos necesarios', 'USUARIO_Controller.php');
                } else {
                    //muestra el formulario de borrado
                    new ESTUDIANTE_SELECT_SHOW($valores,$datos, 'USUARIO_Controller.php');
                }
            } else {
                $_REQUEST['password'] = '';
                $estudiante = get_data_form();
                $respuesta = $estudiante->Borrar();
                new Mensaje($respuesta, 'USUARIO_Controller.php');
            }
        }
